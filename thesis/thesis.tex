...
\begin{document}
  ...
  syntactic sugar allows \lstinline@pythagoras@ to be defined as follows:

  \begin{lstlisting}[gobble=4,basicstyle=\ttfamily\small]
    pythagoras a b = sqrt (a * a + b * b)
  \end{lstlisting}

  While not obvious from the syntax of both previous examples, Haskell is a
  \emph{statically typed} language: a program won't compile if there's a call
  like \lstinline@pythagoras 2 "three"@. In the definition for
