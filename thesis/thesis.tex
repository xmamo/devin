...
\begin{document}
  ...
  Thus, if \lstinline@a@ is an instance of the type class \lstinline@Floating@,
  \lstinline@pythagoras@ is in fact a function of argument \lstinline@x : a@
  which yields a closure of argument \lstinline@y : a@ which evaluates the
  expression \lstinline@sqrt (x ^ 2 + y ^ 2)@; in other words, there is no
  concept of functions with multiple arguments in Haskell! This explains why in
  ...
  for \lstinline@(->) r@. I won't make use such functionality in this thesis.
  ...
  on lists besides the ones exported by \lstinline@Prelude@; one of these is
