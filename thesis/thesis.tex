...
\begin{document}
  ...
  value; thinking about execution as such a rewrite system can help improving
  ...
  performance between the two implementations is negligible.
  ...
  \lstinline@/@, \lstinline@<@, \lstinline@=@, \lstinline@>@, \lstinline@?@,
  \lstinline|@|, \lstinline@\@, \lstinline@^@, \lstinline@|@, \lstinline@-@,
  \lstinline@~@, \lstinline@:@. In fact, given $b$ and $n$, the expression
  \lstinline[mathescape]@$b$ ^ $n$@ has an equivalent prefix form,
  \lstinline[mathescape]@(^) $b$ $n$@. This allows a non-symbolic alias to be
  defined as \lstinline@pow = (^)@, making \lstinline[mathescape]@$b$ ^ $n$@
  equivalent to \lstinline[mathescape]@pow $b$ $n$@. Simmetrically, any binary
  function can be used as an infix operator: given the operator
  \lstinline@(div)@ for integer division towards negative infinity,
  \lstinline[mathescape]@div $n$ $d$@ has an equivalent infix form,
  \lstinline[mathescape]@$n$ `div` $d$@.
  ...
  While lists and tuples are certainly useful, Haskell allows the definition of
  new data types. This form of abstraction improves upon the use of simple
  tuples, as structurally similar objects like 2D vectors and complex numbers
  which are both pairs of numbers, are associated to different types.

  A data type for complex numbers $a + i b$ can be defined as follows:
  ...
  constructor \lstinline@Complex Double Double@. Thus, if $a$ and $b$ are of
  type \lstinline@Double@, the expression
  \lstinline[mathescape]@Complex $a$ $b$@ constructs a new value of type
  \lstinline@Complex@.

  The deriving declaration of the next line instructs the compiler to generate
  three functions: \lstinline@(==), (/=) :: Complex -> Complex -> Bool@ to test
  for equality between values, and \lstinline@show :: Complex -> String@ to
  obtain a string representation of a \lstinline@Complex@.
  ...
  \lstinline@_@ is used as a convention, when its matched value is ignored.
  ...
  When record syntax is used, values can be constructed and pattern matched in
  two ways each. A few functions are provided as to exemplify all synctactic
  possibilities:

  \begin{lstlisting}[gobble=4,basicstyle=\ttfamily\small]
    complexFromPolar :: Double -> Double -> Complex
    complexFromPolar magnitude phase =
      let a = magnitude * cos phase
          b = magnitude * sin phase
       in Complex a b

    complexAdd :: Complex -> Complex -> Complex
    complexAdd (Complex a1 b1) (Complex a2 b2) =
      let a = a1 + a2
          b = b1 + b2
       in Complex a b

    complexMultiply :: Complex -> Complex -> Complex
    complexMultiply (Complex a1 b1) (Complex a2 b2) =
      let a = a1 * a2 - b1 * b2
          b = a1 * b2 + b1 * a2
       in Complex {complexRe = a, complexIm = b}

    complexMagnitude :: Complex -> Double
    complexMagnitude Complex {complexRe = a, complexIm = b} = sqrt (a * a + b * b)
  \end{lstlisting}
  ...
  the academic grades can be defined as follows:
  ...
  Haskell's standard library provides two useful algebraic data types (for
  simplicity, deriving clauses are omitted):
  ...
  Both \lstinline@Maybe@ and \lstinline@Either@ are \emph{type constructors},
  as they are paramaterized by type variables: \lstinline@Maybe@ takes a single
  parameter \lstinline@a@, while \lstinline@Either@ takes two parameters
  \lstinline@a@ and \lstinline@b@. Syntactically, type variables are
  ...
  Both implementations have the same shape:

  \begin{itemize}
    \item If the given list is empty, the base case is reached; some constant
    value is returned.
  ...
  This idiom is called \emph{folding} over a list. The \lstinline@foldr@
  function, which encapsulates this behavior, can be used to implememt both
  \lstinline@sum@ and \lstinline@product@:
  ...
  \begin{lstlisting}[gobble=4,basicstyle=\ttfamily\small]
    ...
    product :: Num a => [a] -> a
    product xs = foldr f 1 xs
      where
        f x acc = x * acc
  \end{lstlisting}

  Lambda syntax can be used in place of local bindings to increase succinctness:
  ...
  Finally, given that \lstinline@(+)@ and \lstinline@(*)@ already denote the
  ...
  As a small note, lists are not the only kind of objects which can be folded
  over. For example, \lstinline@Maybe@ values can be thought of lists which are
  either empty or have exactly one element; as such, they can be folded over as
  well. In general, a type constructor \lstinline@t@ is foldable if there is an
  instance of the function \lstinline@foldr :: (a -> b -> b) -> b -> t a -> b@.
  Given that \lstinline@foldr@ is already provided by the base library for all
  types which can be folded over, \lstinline@sum@ and \lstinline@product@ can be
  defined with more generic signatures as:
  ...
  Besides \lstinline@foldr@, another commonly used higher order function is
  \lstinline@map@. Given a list, \lstinline@map@ returns a new list obtained by
  applying a given input function to all elements of the list. \lstinline@map@
  can be implemented in terms of \lstinline@foldr@:
  ...
  One final higher order function to know about is the \emph{application
  operator} \lstinline@($)@.
