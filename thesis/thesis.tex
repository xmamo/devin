...
\begin{document}
  ...
  As an example, consider the function to compute the Levenshtein distance
  between two strings; that is, the minimum number of single-character edits
  (insertions, deletions or substitutions) required to change one string into
  the other. The complexity of a na\"ive recursive implementation is
  ...
  display the current runtime stack, as seen in
  ...
  called, a new frame is pushed onto the stack; it contains associations
  ...
  Devin's implementation spans approximately 3500 lines of code and is entirely
  ...
  of different types are deemed to be different. Thus, \lstinline@==@ and
  \lstinline@!=@ are type checked as follows:
  ...
  checks variable and function definitions, adding new bindings to the
  environment.
  ...
  meant to work with both arrays and numbers. For brevity, only the
  ...
  evaluates statements; its result is \lstinline@Just@ on statements returning
  control, \lstinline@Nothing@ otherwise.

  The following two base cases are trivial (\lstinline@evalDefinitions@'s
  implementation is discussed in the next section):

  \begin{lstlisting}[gobble=4,basicstyle=\ttfamily\small]
    evalStatement DefinitionStatement{definition} = do
      evalDefinition definition
      pure Nothing

    evalStatement ExpressionStatement{value} = do
  ...
  \begin{lstlisting}[gobble=4,basicstyle=\ttfamily\small]
    evalStatement IfStatement{predicate, trueBranch} = do
  ...
  \begin{lstlisting}[gobble=4,basicstyle=\ttfamily\small]
    ...
    evalStatement IfElseStatement{predicate, trueBranch, falseBranch} = do
      r <- evalExpression predicate
      v <- readRef r
  ...
  \begin{lstlisting}[gobble=4,basicstyle=\ttfamily\small]
    evalStatement ReturnStatement{result = Just result} = do
      r <- evalExpression result
      pure (Just r)

    evalStatement ReturnStatement{result = Nothing} = do
      r <- newRef Unit
      pure (Just r)
  \end{lstlisting}
  ...
  \begin{lstlisting}[gobble=4,basicstyle=\ttfamily\small]
    evalStatement BlockStatement{statements} = withNewFrame Nothing 1 $ do
      for_ statements $ \case
  ...
  \lstinline@s@ is the \linebreak

  \pagebreak

  underlying \lstinline@Steam@ (either \lstinline@String@ or \lstinline@Text@),
  ...
  Devin's UI implementation is complex, requiring almost 1000 lines of code.
  ...
  \begin{itemize}
    ...
    output of Devin's parser. GtkSourceView operates on such an abstraction level
