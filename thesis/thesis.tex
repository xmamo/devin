...
\begin{document}
  ...
  \lstinline[mathescape]@(^) $b$ $n$@; a non-symbolic alias can be defined as
  \lstinline@pow = (^)@, making \lstinline[mathescape]@$b$ ^ $n$@ equivalent to
  \lstinline[mathescape]@pow $b$ $n$@. Simmetrically, any binary function can be
  used as an infix operator: given the operator \lstinline@(div)@ for integer
  division towards negative infinity, \lstinline[mathescape]@div $n$ $d$@ has an
  equivalent infix form, \lstinline[mathescape]@$n$ `div` $d$@.
  ...
  words, if $a$ and $b$ are of type \lstinline@Double@, the expression
  ...
  frequently employ this operator; to prevent confusion, I'll avoid using it
  unless necessary.
  ...
  provided, then both will loop. Most predefined datatypes are instance of this
  class.
