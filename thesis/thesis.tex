...
\begin{document}
  ...
  haskell's standard library providesmany functions operating on lists. Some
  commonly used are:
  ...
  The standard library exports very useful function operating on association
  ...
  of the list. For lists, \lstinline@fmap@ can be implemented in terms of
  \lstinline@foldr@:
  ...
  Haskell's standard library \lstinline@base@ exposes many other higher order
  functions and operators; these are all documented at
  ...
  programs working with multitudes of data types would need to employ lots of
  ...
  \lstinline@const x _ = x@ as defined Haskell's standard library,
  ...
  With \lstinline@do@ notation, programs of arbitrary complexity can be written.
  In fact, as long as imperative loops aren't required, programming in Haskell
  is even more succint than languages which mainly focus on imperative style
  programming:
  ...
  While imperative style programming is always an option, it should be preferred
  to code in other ways, as Haskell doesn't have any syntax for constructs such
  as while- or for-loops; even if higher order functions can be used to get
  equivalent functionality, it is often times clumsy doing so.
  ...
  exact meaning of the provided function signatures, especially considering that
  \lstinline@runST@ employs something called \emph{existensials} through the
  keyword \lstinline@forall@. Apart fromm this example, I won't use or mention
  existentials again.
  ...
  The function \lstinline@factorialST@ may be read as follows: create a new
