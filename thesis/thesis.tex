...
\begin{document}
  ...
  be more succinctly rewritten as:
  ...
  to avoid writing needlessly cryptic code, I won't be using it unless strictly
  necessary.
  ...
  chained together with \lstinline@(>>=)@.
  ...
  entities exported by the \lstinline@Data.List@ module. Further, as explained
  in \autoref{section:strings}, the function \lstinline@words@  splits a string
  on white space; its dual, \lstinline@unwords@, joins a list of strings with
  separating spaces.
  ...
  called \lstinline@Triangle2D@ which exports types and functions to operate on
  triangles situated on the Euclidean plane. Such module would be defined in its
  own file, \lstinline@Triangle2D.hs@; one possible implementation is given by
  ...
  one triangle equal the vertices of the other, independently of their order; to
  do so, the functions
  ...
  How would such function operate without builtin type classes? The key insight
  ...
  given substring, as shown in \autoref{figure:cst}.
  ...
  \lstinline[mathescape]@intToFloat($n$)@ and
  \lstinline[mathescape]@floatToInt($x$)@.
  ...
  a global function named \lstinline@main@; it must take no arguments.
  ...
  brevity. In particular, import declarations and \lstinline@LANGUAGE@ pragmas
  are left out for simplicity. Note that some of the functionality of the
  package \lstinline@extra@ is used: the library provides extra functions for
  the standard Haskell libraries, filling out missing
  ...
  The pair of unary operators \lstinline@+@ and \lstinline@-@ and the 20 binary
  operators \lstinline@+@, \lstinline@-@, \lstinline@*@, \lstinline@/@,
  \lstinline@%@, \lstinline@==@, \lstinline@!=@, \lstinline@<@, \lstinline@<=@,
  \lstinline@>@, \lstinline@>=@, \lstinline@and@, \lstinline@or@,
  \lstinline@xor@, \lstinline@=@, \lstinline@+=@, \lstinline@-=@,
  \lstinline@*=@, \lstinline@/=@, \lstinline@%=@ are represented by the
  algebraic data types \lstinline@UnaryOperator@ and \lstinline@BinaryOperator@
  respectively:
  ...
  \lstinline@open@ and \lstinline@close@ fields, regardless of the kind: round,
  square or curly.
  ...
  statements are represented by the algebraic data type \lstinline@Statement@,
  along with if, if-else, while, do-while, return, assert, breakpoint, and block
  statements.
  ...
  Type checking is performed by a set of functions operating on an
  \emph{environment}: a data structure which associates identifiers to types.
  The environment is used to verify if function calls and variable usages are
  valid within their context.

  The module \lstinline@Devin.Type@ provides the data types necessary to
  represent types themselves. The data constructors \lstinline@Unit@,
  \lstinline@Bool@, \lstinline@Int@, \lstinline@Float@ and \lstinline@Array@
  correspond to Devin's supported types. To aid the type checker, two other data
  constructors are provided, both of which indicate some sort of error:
  \lstinline@Unknown@ and \lstinline@Placeholder@. While \lstinline@Unknown@
  stands for an unknown type, \lstinline@Placeholder@ represents a specific
  unresolved type. Differentiating between between \lstinline@Placeholder@ and
  \lstinline@Unknown@ allows for fine-grained error messages and diagnostics.
  For instance, if a misspelled type occurs the same way $n$ times within some
  scope, it corresponds to \emph{one}, instead of $n$, type errors.
  ...
  Some utility functions are defined. The function \lstinline@defineType@ binds
  a type name (such as \lstinline@"Unit"@) to a \lstinline@Type@ (such as
  ...
  \lstinline[mathescape]@\case { $p_1$ -> $e_1$; $\ldots$; $p_n$ -> $e_n$ }@ is
  called a \emph{lambda-case}; it is equivalent to
  \lstinline[mathescape]@\$x$ -> case $x$ of { $p_1$ -> $e_1$; $\ldots$; $p_n$ -> $e_n$ }@,
  where $x$ is a fresh variable. Lambda-case syntax is enabled by the
  \lstinline@LambdaCase@ language extension.

  Next, the functions \lstinline@defineFunSignature@ and
  \lstinline@lookupFunSignature@ define and lookup associations between names
  and function signatures, represented by a \lstinline@([Type], Type)@. The
  first element of the tuple is the list of parameter types, the second is the
  return type.
  ...
  It is similar to \lstinline@report@, but its result wraps \lstinline@Unknown@
  instead of the unit type \lstinline@()@.
  ...
  there are at least two elements of the array with different types, then $T$
  can't be inferred.
  ...
  \lstinline@loopM :: Monad m => (a -> m (Either a b)) -> a -> m b@ expresses a
  monadic loop operation, where the predicate returns \lstinline@Left@ as a seed
  for the next loop or \lstinline@Right@ to abort the loop. To apply
  \lstinline@loopM@ with flipped argument order,
  \lstinline@flip :: (a -> b -> c) -> b -> a -> c@ is used.
  ...
  considered. For instance, while both binary operators \lstinline@+@ and
  \lstinline@-@ operate on arithmetic types, \lstinline@+@ also works with
  arrays. Worse, in the evaluator, both \lstinline@+@ and \lstinline@and@
  compute some result based on their operands, but the evaluation strategy of
  \lstinline@and@ differs from that of \lstinline@+@. Source code could
  ...
  The binary operators \lstinline@-@ and \lstinline@%@ are type checked
  similarly to \lstinline@/@, except for the fact that
  ...
  \lstinline@>=@. Type checking ensures that both operands are of the same type;
  if they are, expressions with such operators yield a \lstinline@Bool@; as
  we'll see, the semantic meaning of comparing non-numeric values is
  lexicographic ordering. Type checking is implemented in the exactly same way
  ...
  The operators \lstinline@and@, \lstinline@or@ and \lstinline@xor@ are all type
  ...
  \lstinline@foldlM :: (Foldable t, Monad m) => (b -> a -> m b) -> b -> t a -> m b@
  is just an analogous to \lstinline@foldl@ (which is a left-associative version
  of \lstinline@foldr@ (\autoref{section:higher-order-functions})), except that
  ...
  \lstinline@checkDefinition2 :: Definition -> Typer ()@ type checks variable
  and function definitions, adding new bindings to the environment.
  ...
  The data types and functions defined in the \lstinline@Devin.Evaluator@ module
  implement the evaluator interface. Evaluation operates on a linked list of
  \emph{frames}; each frame holds bindings between function names and their
  ...
  A \lstinline@Function@ is either be a Devin builtin with ad-hoc semantics, or
  ...
  multiple identifiers can refer to the same value. Devin's \lstinline@Cell@
  data type wraps Haskell's \lstinline@IORef@, which provides mutable references
  in the \lstinline@IO@ monad. Devin arrays are represented as
  \lstinline@Vector Cell@ instead of \lstinline@[Cell]@, as vector access is
  $O\mathopen{}\left(1\right)\mathclose{}$.
  ...
  To operate on cells, the \lstinline@newCell@, \lstinline@readCell@ and
  \lstinline@writeCell@ functions are provided. These create, read from, and
  ...
  Above, \lstinline@MonadIO m@ stands for the class of monads in which
  \lstinline@IO@ computations may be embedded; the function
  \lstinline@liftIO :: MonadIO m => IO a -> m a@ lift a computation from the
  \lstinline@IO@ monad. This abstraction allows I/O operations to be run in many
  monadic stacks.

  The functions \lstinline@cloneCell@ and \lstinline@cloneVal@ clone cells and
  values respectively. As we'll see, these are needed to implement the pass by
  value evaluation strategy. To clone arrays,
  \lstinline@Vector.forM :: Monad m => Vector a -> (a -> m b) -> m (Vector b)@
  ...
  \lstinline@compareCells@ and \lstinline@compareVals@. Array elements are
  accessed using \lstinline@(!) :: Vector a -> Int -> a@. Notice that
  ...
  \lstinline@makePredefinedState@. The 2-tuple constructor \lstinline@(,)@ is
  used explicitly so that it can be applied partially.
  ...
  Execution is abstracted by the \lstinline@Evaluator@ data type; it represents
  a computation which updates a \lstinline@State@, possibly producing some
  \lstinline@IO@ side-effect:
  ...
  algebraic data type \lstinline@Result@ is provided. It has three constructors:

  \begin{itemize}
    \item \lstinline@Done@, if the last statement has been executed. This data
    constructor stores the result of the computation.

    \item \lstinline@Yield@, if a statement has been reached. The statement,
  ...
  To use \lstinline@Evaluator@s monadically, instances of
  \lstinline@Applicative@ and \lstinline@Monad@ are provided:
  ...
  An instance for \lstinline@MonadIO@ is provided as well. This instance
  permits, among others, \lstinline@newCell@, \lstinline@readCell@,
  \lstinline@writeCell@, \lstinline@cloneCell@, \lstinline@compareCells@,
  ...
  done with the \lstinline@poffset@ field, which stands for \emph{parent
  ...
  \begin{itemize}
    ...
    like these that the \lstinline@poffset@ field needs to be used.
  \end{itemize}
  ...
  The expressions \lstinline@frame{funs = funs'}@ and
  \lstinline@frame{vars = vars'}@ used in the functions \lstinline@defineFun@
  and \lstinline@defineVar@ denote \emph{record updates}: the record
  \lstinline@frame@ is copied, updating the fields \lstinline@funs@ and
  \lstinline@vars@ respectively.

  To run another \lstinline@Evaluator@ on a modified state where a new
  \lstinline@Frame@ is added, \lstinline@withNewFrame@ is provided; when the
  function returns, the previous state is restored.
  ...
  Finally, the functions \lstinline@yield@ and \lstinline@raise@ are provided
  for convenience.
  ...
  \lstinline@evalExpression :: Expression -> Evaluator Cell@ evaluates
  ...
  function, using
  \lstinline@Vector.unfoldrM :: Monad m => (b -> m (Maybe (a, b))) -> b -> m (Vector a)@.
  The generator function yields \lstinline[mathescape]@Just ($x$, $y$)@ or
  \lstinline@Nothing@ if there are no more elements, where $x$ is the next
  ...
  the the expression involves invalid types, a runtime exception is raised.
  ...
  The binary operator \lstinline@(!?) :: Vector a -> Int -> Maybe a@ is used to
  safely index vectors. It is closely related to
  \lstinline@(!) :: Vector a -> Int -> a@; if the index is out of bounds, it
  returns \lstinline@Nothing@ instead of throwing an exception.

  Arithmetic expressions may lead to overflows. The higher-order helper
  functions \lstinline@safeUnary@ and \lstinline@safeBinary@ are used to wrap
  Haskell's builtin operators, allowing for overflow detection. Both
  ...
  \lstinline@Integer@; its inverse \lstinline@toIntegralSized@ attempts to
  convert an \lstinline@Integer@ back to an \lstinline@Int64@. If it is not
  possible to fit an \lstinline@Integer@ in the 64 bits of an \lstinline@Int64@,
  \lstinline@toIntegralSized@ yields \lstinline@Nothing@.
  ...
  operator is \lstinline@+@, nothing else needs to be done.
  ...
  The binary operators \lstinline@+@ and \lstinline@*@ are overloaded to work
  with both arrays and numbers. For \lstinline@+@, the operands must either be
  both numbers or both arrays; for \lstinline@*@, the operands must be either
  both numbers or one a number and one an array. For brevity, only the
  ...
  \begin{enumerate}
    \item The lengths $n_1$ and $n_2$ of the arrays are used to compute the
    length $n_3$ of the new array which will contain the elements of the first
    array concatenated with the second;
  ...
  \lstinline@>@, \lstinline@>=@ are implemented using \lstinline@compareCells@.
  Note that an \lstinline@Ordering@ is itself an instance of \lstinline@Ord@,
  making the implementation trivial for all operators:
  ...
  second operand. Thus, their implementation differs from that of other binary
  operators such as \lstinline@xor@.
  ...
  \begin{lstlisting}[gobble=4,basicstyle=\ttfamily\small]
    evalExpression expression@BinaryExpression{left, binary, right} | DivideAssignOperator{} <- binary = do
      leftCell <- evalExpression left
      leftVal <- readCell leftCell
  ...
  many or to few arguments are passed, an exception is raised. Builtin functions
  are handled according to their meaning: \lstinline@not@ negates a boolean,
  \lstinline@len@ returns the length of an array, \lstinline@toFloat@ and
  \lstinline@toInt@ convert an \lstinline@Int@ to a \lstinline@Float@ and round
  a \lstinline@Float@ to an \lstinline@Int@ respectively. User-defined function
  calls are evaluated by binding parameter names to the evaluated arguments and
  then recursively evaluating the callee's body. If a given argument is passed
  by value, a copy of its cell is bound; if it is passed by reference, no copy
  is performed. When a user-defined function's body is evaluated, a new frame is
  pushed by calling \lstinline@withNewFrame (depth + 1)@, where
  \lstinline@depth@ is the nesting level of the callee with respect to the
  caller.
  ...
  The function
  \lstinline@evalStatement :: Statement -> Evaluator (Maybe Cell)@ evaluates
  statements; its result is \lstinline@Just@ on statements returning control,
  \lstinline@Nothing@ otherwise.
  ...
  \lstinline@Parsec (Int, s) [Token] a@, where: \lstinline@Int@ is the offset,
  \lstinline@s@ is the underlying \lstinline@Steam@ (either \lstinline@String@
  or \lstinline@Text@), \lstinline@[Token]@ is the list of comments accumulated
  in the state, \lstinline@a@ is the result of the parser.
  ...
  \lstinline@char '.'@, is ignored by employing \lstinline@(*>)@: this
  ...
  listing, is equivalent to \lstinline[mathescape]@$p$ *> pure $q$@.
  ...
  any previous label is discarded.
  ...
  The \lstinline@choice@ combinator tries to apply the parsers in a list in
  order, until one of them succeeds; the value provided by parser which succeeds
  is returned. Such combinator is equivalent to \lstinline@foldr (<|>) empty@.
  ...
  \begin{itemize}
    ...
    there's builtin support for showing line numbers, automatic indentation, and
    syntax highlighting.
    ...
    C isn't exposed by \lstinline@gi-gtk@. One example of such missing
    ...
    GTK+ and \lstinline@gi-gtk@. One of the tricks I employed is to use a subset
    of GTK+ functionality, exposed through \lstinline@haskell-gi-base@ (a
    dependency of \lstinline@gi-gtk@), which allows for \lstinline@GObject@s
    (such as widgets) to be instantiated in a different way.
  \end{itemize}
  ...
  \lstinline@Text@ data type instead of \lstinline@String@ for efficiency; to
  convert between the two, \lstinline@Text.pack :: String -> Text@ and
  ...
  illustrates an alternative \lstinline@onActivate@ implementation employing
  such functionality:
  ...
  parameters builtin; type classes thus emerge
  ...
  \lstinputlisting
    [basicstyle=\ttfamily\footnotesize,aboveskip=0pt,belowskip=\baselineskip]
    {../app/Devin/Debug/Evaluator.hs}

  \section{\texttt{app/Devin/Debug/Syntax.hs}}

  \lstinputlisting
    [basicstyle=\ttfamily\footnotesize,aboveskip=0pt,belowskip=\baselineskip]
    {../app/Devin/Debug/Syntax.hs}
  ...
  \lstinputlisting
    [basicstyle=\ttfamily\footnotesize,aboveskip=0pt,belowskip=\baselineskip]
    {../app/Devin/Highlight/Brackets.hs}
