...
\begin{document}
  ...
  syntax trees for the string \lstinline@2+3*(4+5)@, depending on how much
  ...
  while loops, do-while loops and brancing with if and if-else statements.
  ...
  Booleans support the usual operations of negation, conjunction and
  disjunction. Additionaly, an operator for exclusive disjunction is provided.
  ...
  If-else statements are type checked by ensuring that the predicate is a
  boolean expression and recursively type checking both possible execution
  paths. We follow a conservative approach and verify if both branches return
  control; in such case, the whole if-else statement surely must return control
  as well.
  ...
  Finally, blocks are type checked by recursively type checking all statements
  they contain. It is deduced that a block returns control if at least one of it
  statements does.
  ...
  checks variable and function definitions, adding a new binding of the symbol's
  ...
  \begin{itemize}
    ...
    like these that the \lstinline@offset@ field is used to recover the actual
    lexical parent.
  \end{itemize}
  ...
  \lstinline@evalExpression :: Expression -> Evaluator Reference@ evaluates an
  expression and returns the cell containing the resulting value.
  ...
  \begin{lstlisting}[gobble=4,basicstyle=\ttfamily\small]
    evalExpression expression | IntegerExpression {integer} <- expression =
  ...
  are raised if subexpressions evaluate to the wrong type.
  ...
  and then executing the callee's body. If a given argument is passed by value,
  a copy of its cell is bound; otherwise, no copy is performed.
  ...
  evaluated. With \lstinline@firstJustM@, execution stops at the first return
  statement, as only return statements produce a \lstinline@Just@.
  ...
  in both cases; however, implementation is so trivial that there's little
  benefit in refactoring.
  ...
  Devin only misses a module system and a standard library. With the basic
  ...
  I demonstrated how a monadic mechanism can be used to represent pausable
  ...
  \lstinputlisting[basicstyle=\ttfamily\small]{../devin.cabal}

  \section{\texttt{src/Devin/Display.hs}}

  \lstinputlisting[basicstyle=\ttfamily\small]{../src/Devin/Display.hs}

  \section{\texttt{src/Devin/Error.hs}}

  \lstinputlisting[basicstyle=\ttfamily\small]{../src/Devin/Error.hs}

  \section{\texttt{src/Devin/Evaluator.hs}}

  \lstinputlisting[basicstyle=\ttfamily\small]{../src/Devin/Evaluator.hs}

  \section{\texttt{src/Devin/Evaluators.hs}}

  \lstinputlisting[basicstyle=\ttfamily\small]{../src/Devin/Evaluators.hs}

  \section{\texttt{src/Devin/Interval.hs}}

  \lstinputlisting[basicstyle=\ttfamily\small]{../src/Devin/Interval.hs}

  \section{\texttt{src/Devin/Parsec.hs}}

  \lstinputlisting[basicstyle=\ttfamily\small]{../src/Devin/Parsec.hs}

  \section{\texttt{src/Devin/Parsers.hs}}

  \lstinputlisting[basicstyle=\ttfamily\small]{../src/Devin/Parsers.hs}

  \section{\texttt{src/Devin/Syntax.hs}}

  \lstinputlisting[basicstyle=\ttfamily\small]{../src/Devin/Syntax.hs}

  \section{\texttt{src/Devin/Type.hs}}

  \lstinputlisting[basicstyle=\ttfamily\small]{../src/Devin/Type.hs}

  \section{\texttt{src/Devin/Typer.hs}}

  \lstinputlisting[basicstyle=\ttfamily\small]{../src/Devin/Typer.hs}

  \section{\texttt{src/Devin/Typers.hs}}

  \lstinputlisting[basicstyle=\ttfamily\small]{../src/Devin/Typers.hs}

  \section{\texttt{src/Devin/Utils.hs}}

  \lstinputlisting[basicstyle=\ttfamily\small]{../src/Devin/Utils.hs}

  \section{\texttt{test/Main.hs}}

  \lstinputlisting[basicstyle=\ttfamily\small]{../test/Main.hs}

  \section{\texttt{test/Devin/EvaluatorsSpec.hs}}

  \lstinputlisting[basicstyle=\ttfamily\small]{../test/Devin/EvaluatorsSpec.hs}

  \section{\texttt{test/Devin/ParsersSpec.hs}}

  \lstinputlisting[basicstyle=\ttfamily\small]{../test/Devin/ParsersSpec.hs}

  \section{\texttt{test/Devin/TypersSpec.hs}}

  \lstinputlisting[basicstyle=\ttfamily\small]{../test/Devin/TypersSpec.hs}

  \section{\texttt{app/Main.hs}}
  \label{section:app-main-hs}

  \lstinputlisting[basicstyle=\ttfamily\small]{../app/Main.hs}

  \section{\texttt{app/Devin/Highlight.hs}}

  \lstinputlisting[basicstyle=\ttfamily\small]{../app/Devin/Highlight.hs}

  \section{\texttt{app/Devin/Tree.hs}}

  \lstinputlisting[basicstyle=\ttfamily\small]{../app/Devin/Tree.hs}

  \section{\texttt{app/Devin/Highlight/Braces.hs}}

  \lstinputlisting[basicstyle=\ttfamily\small]{../app/Devin/Highlight/Braces.hs}

  \section{\texttt{app/Devin/Highlight/Syntax.hs}}

  \lstinputlisting[basicstyle=\ttfamily\small]{../app/Devin/Highlight/Syntax.hs}
