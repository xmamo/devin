...
\begin{document}
  ...
  \begin{itemize}
    ...
    \item Otherwise, a function is applied to head of the list and the recursive
    function call.
  \end{itemize}

  The \lstinline@foldr@ function
  ...
  %     (x : xs) =
  %       let (xs1, xs2) = partition (< x) xs
  %        in (sortList xs1) ++ [x] ++ (sortList xs2)
  % \end{lstlisting}
