...
\begin{document}
  ...
  A \lstinline@Function@ represents a bound function. It can either be a Devin
  ...
  Values are not stored directly inside \lstinline@State@. Mirroring LISP's
  implementation, each identifier is bound to a cell which holds the actual
  value. This way, multiple identifiers can refer to the same value. In Devin,
  cells are implemented by the \lstinline@Reference@ data type; it wraps
  Haskell's \lstinline@IORef@.
  ...
  more general Unicode definition allows characters like
  \lstinline[literate={è}{\`e}{1}]@è@ or \lstinline[literate={ß}{\ss}{1}]@ß@ to
  be accepted as well.
