...
\begin{document}
  ...
  ordered types called \lstinline@Ord@. It is defined as follows:
  ...
  \lstinline@Ord@, since \lstinline@let nan = 0 / 0 in nan <= nan@ evaluates to
  \lstinline@False@. Because some laws are somewhat difficult to understand, and
  because even Haskell itself ignores these with unlawful instances, I'll avoid
  mentioning other laws to prevent confusion. As it happens, one must really try
  hard to come up with a datatype with unlawful operations, like the IEEE 754
  committee did; in most cases, implementing a type class instance satisfies its
  laws without thinking about them. Still, if the reader is interested in
  reading about these laws, documentation for Haskell's \lstinline@Prelude@
  ...
  The definitions for the \lstinline@Applicative@ and \lstinline@Monad@ classes
  are shown in the listing below. As a small note, the official Haskell language
  report doesn't define the \lstinline@Applicative@ class; further, it doesn't
  define any relationship between any of these classes. Since \lstinline@base@
  is the library which is used to develop applications in practice, its
  definitions are shown instead.
